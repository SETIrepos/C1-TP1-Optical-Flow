...
\subsubsection{Hypothèse et formulation du problème}
...
\subsubsection{Discrétisation des dérivées (Section 7)}
...
\subsubsection{Laplacien et terme de lissage (Section 8)}
...
\subsubsection{Schéma final d’itération (Section 12)}
...
\subsubsection{Détection simple des objets en mouvement}

Pour détecter si un pixel est en mouvement entre 2 images, on peut calculer les magnitude du flot optique, autrement dit la "force" avec laquelle le pixel bouge entre les 2 images et on peut comparer cette magnitude avec la moyenne des magnitudes de chaque pixel de l'image.\\
...
\subsection{Implémentation et analyse des résultats}

\subsubsection{Implémentation du code}

Le code complet est disponible sur le \href{https://github.com/SETIrepos/C1-TP1-Optical-Flow.git}{repo GitHub du projet}. Il est implémenté en Matlab car les fonctions \verb|flowToColor.m| et \verb|computeColor.m| nous étaient déjà fournies.\\

On créé donc une fonction \verb|horn_schunck(I1, I2, alpha, n_iter, gauss_sigma)| qui prend en argument:

\begin{itemize}
    \item \verb|I1| et \verb|I2| 2 images successives en niveau de gris
    \item \verb|alpha| le paramètre de régularisation (10 par défaut)
    \item \verb|n_iter| le nombre d'itérations pour la boucle de mise à jour (pour des images simple l'état de l'art recommande 100 à 300 itérations mais peut augmenter avec la taille de l'image)
    \item \verb|gauss_sigma| la valeur du flou gaussien que l'ont va appliquer (0.8 est convenable)\\
\end{itemize}

\textit{Remarque:} plus \verb|alpha| est élevé, plus la convergence sera lente et plus il faudra augmenter \verb|n_iter|.\\


En sortie on recupère \verb|[u, v]| qui sont les valeurs pour chaque point de l'image du déplacement du flot optique selon $\Vec{x}$ et $\Vec{y}$.\\

Pour tracer le flot optique de manière analogue à l'article (sous forme de flèches) on peut créer une fonction \verb|plotFlowArrows.m| qui va tracer les flèches correspondantes au flot et pouvoir voir l'effet des modification d'alpha sur le flot optique car \verb|flowToColor| normalise les valeurs ce qui n'est pas représentatif.\\

Voici ce que donne l'application de l'algorithme avec \verb|alpha = 10| et \verb|n_iter = 200|:

\begin{figure}[h!]
    \centering
    \includegraphics[width=1\linewidth]{flow12_alpha10.png}
    \caption{Flot optique pour $\alpha = 10$}
    \label{fig:placeholder}
\end{figure}

\subsubsection{Impact de $\alpha$}

Si l'on modifie alpha le sens des flèches n'est pas modifié mais leur longueur va évoluer, plus alpha est grand plus les flèches seront régulières dans les zones de mouvement et lorsque alpha est plus petit les flèches sont plus petites et plus chaotiques.
...
\begin{figure}[h!]
    ...
    \begin{minipage}{0.45\textwidth}
        \centering
        \includegraphics[width=\linewidth]{flow12_alpha01.png}
        \caption{$\alpha = 0.1$}
    \end{minipage}
    \hfill
    % deuxième image
    \begin{minipage}{0.45\textwidth}
        \centering
        \includegraphics[width=\linewidth]{flow12_alpha100.png}
        \caption{$\alpha = 100$}
    \end{minipage}
\end{figure}

\subsubsection{Impact du nombre d'itérations}

Plus il y a d'itération plus on détectera des variations plus faibles du flot optique avec le risque de ne pas tout détecter si le nombre est trop faible et de trop détecter avec un nombre trop élevé.
...
\begin{figure}[h!]
    ...
    \begin{minipage}{0.45\textwidth}
        \centering
        \includegraphics[width=\linewidth]{n1000.png}
        \caption{1000 itérations}
    \end{minipage}
\end{figure}

\subsubsection{Détection des objets en mouvement}

Pour détecter les objets en mouvement on se base sur la magnitude du flot optique que l'on compare pour chaque point à la moyenne.\\
...
\begin{figure}[h!]
    ...
    \begin{minipage}{0.45\textwidth}
        ...
        \caption{Masque des objets en mouvement}
    \end{minipage}
\end{figure}

\subsubsection{Détection finale}

Lorsque que l'ont prend un nombre d'itérations convenable (300), une valeur d'alpha moyenne (10), on arrive à trouver un bon compromis entre le temps d’exécution et la précision:
...
\begin{figure}[h!]
    ...
    \begin{minipage}{0.3\textwidth}
        \centering
        \includegraphics[width=\linewidth]{flowColor.png}
        \caption{Flot colorisé via $flowToColor$}
    \end{minipage}
    \hfill
    % troisième image
    \begin{minipage}{0.3\textwidth}
        \centering
        \includegraphics[width=\linewidth]{contour.png}
        \caption{Détection des objets en mouvement}
    \end{minipage}
\end{figure}
